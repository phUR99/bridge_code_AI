...
import utils
# LLM 클래스 가져오기
from llm import LLM, gen, topic_gen
...
def interpret_initial_code_api(request: InitialQueryRequest):
    ...
    """
    global db, llm, data, code
    # API 토큰 검증
    
    sample_data = data.sample(1, ignore_index=True)
    topic = sample_data['topic'].astype(str)
    question = sample_data['question'].astype(str)
    #comment = sample_data['comment'].astype(str)
    #print(topic)
    #print(question)
    code = question[0]
    
    input_list = utils.str_to_list(str(question))
    #  1. 유저 입력을 임베딩
    ...
    #print(response)
    return {"topic":topic[0], "origin":question[0], "response":response}
